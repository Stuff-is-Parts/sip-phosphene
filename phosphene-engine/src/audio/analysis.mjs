...
export class Analysis {
  ...
  #avgBass = 0.01; #cooldown = 0; #beatTimes = [];
  #freq = null; #time = null; #analyser = null;
  ...
  inject(nowSec) { this.beat = 1; this.#pushBeat(nowSec); }
  #pushBeat(nowSec) {
  ...
  update(nowSec) {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      let s = 0; for (let j = a; j < b; j++) s += f[Math.min(j, n - 1)];
