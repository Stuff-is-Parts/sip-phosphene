...
| Item | Source | PHOSPHENE representation | PHOSPHENE implementation | Semantic test | Status | Notes |
|---|---|---|---|---|---|---|
| Preset base value defaults (60+ keys) | Butterchurn `node_modules/butterchurn/lib/butterchurn.js:6505-6574` (Visualizer.baseValsDefaults) | `src/core/milk-runner.ts` `MILK_BASE_DEFAULTS` | same | `tests/milk-runner.test.ts` runtime-defaults-exhaustive block — every key in the source table asserted equal to the same key in `MILK_BASE_DEFAULTS` | implemented | |
| Shape base value defaults | Butterchurn `butterchurn.js:6575-6600` (Visualizer.shapeBaseValsDefaults) | `MILK_SHAPE_DEFAULTS` | same | `tests/milk-runner.test.ts` runtime-defaults-exhaustive block | implemented | |
| Wave base value defaults | Butterchurn `butterchurn.js:6601-6615` (Visualizer.waveBaseValsDefaults) | `MILK_WAVE_DEFAULTS` | same | `tests/milk-runner.test.ts` runtime-defaults-exhaustive block | implemented | |
...
| Comp grid hue-color interpolation with rand_start-seeded oscillators | Butterchurn `comp/comp.js` `generateHueBase` + `generateCompColors`; extracted at `src/gpu/milk-pipeline.ts` `generateHueBase` (exported) | `src/gpu/milk-pipeline.ts` `generateCompColors` calls `generateHueBase` | same | `tests/milk-comp.test.ts` — 12-element output shape, max-shade rescale to [0.5, 1.0], randStart[3]/[1]/[2] map to r/g/b (not [0]), time reproducibility, controlled-input golden math | partial | Missing: golden vertex-buffer test asserting the (COMP_W+1)*(COMP_H+1) bilinear-interpolated grid produces the source-witnessed vertex layout for a controlled input. |

## 8. Pass ordering and pipeline

| Item | Source | PHOSPHENE representation | PHOSPHENE implementation | Semantic test | Status | Notes |
|---|---|---|---|---|---|---|
| Frame pipeline order (frame eqs → motion vectors → previous-flip → warp → blur → shapes → waves → basic wave → darken center → borders → composite → swap) | projectM `MilkdropPreset.cpp` `RenderFrame`; Butterchurn `rendering_renderer.js` render(); documented in `milkdrop-execution-model.md` §1 | `src/core/graph.ts` graph.order + `src/gpu/milk-pipeline.ts` switch dispatch | same | `tests/milk-graph-order.test.ts` — full stage sequence assertion, shapes in numeric index order, waves in numeric index order before defaultWave, warp immediately follows frame, borders immediately precedes composite, composite is last | partial | Missing: blur-stage-presence assertion when a warpShader or compShader is present. |
...
| Item | Source | PHOSPHENE representation | PHOSPHENE implementation | Semantic test | Status | Notes |
|---|---|---|---|---|---|---|
| MilkDrop 2 warp shader (HLSL → WGSL) | projectM `PresetShaderHeaderGlsl330.inc` (bindings) + source HLSL forms in the corpus | `src/transpile/hlsl.ts` `hlslToBody("warp")` — partial transpiler that emits WGSL body from HLSL, wired at `src/import/milk.ts:299-303` for the scene-conversion path but NOT at `src/import/milk-graph.ts` for the graph-executor path | `MilkPipeline.load()` still refuses `warpShader` (see refused surfaces below) | none | unresolved | 80 of 97 corpus presets refuse at load. The transpiler `src/transpile/hlsl.ts` exists but its blur builtins (`mdgetblur1/2/3`, lines 145-169) return a fixed-radius 5-tap Gaussian approximation over the main framebuffer, not a true multi-pass Gaussian cascade — the source comment names this as an honest approximation (line 152-153). Per COMPATIBILITY-GOAL.md ("behavior whose semantics are not yet established must be refused rather than approximated"), the row stays unresolved until (a) the true multi-pass blur cascade lands as a separate stage and (b) the transpiler wires through the milk-graph path with the correct blur bindings. |
| MilkDrop 2 comp shader (HLSL → WGSL) | same | same transpiler, `hlslToBody("comp")` | `MilkPipeline.load()` still refuses `compShader` | none | unresolved | 79 presets. Same limitation as above — blur builtins are approximations, and the graph-executor path does not consume the transpiler. |
| Blur cascade (3 levels: 8-tap horizontal → 4-tap vertical → next level) | projectM `Blur1FragmentShaderGlsl330.frag`, `Blur2...`, `Blur3...` — three separable Gaussian stages producing 3 downsampled textures | not represented in the graph as separate stages | not implemented — approximated inline in the transpiler | none | unresolved | 66 presets. The blur cascade is a MULTI-PASS rendering pipeline (separate render targets per level, separable H/V passes each), not a per-fragment approximation. Retiring the inline approximation and adding the real cascade is the direct implementation path. |
