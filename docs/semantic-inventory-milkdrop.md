...
| gmegabuf shared across all runtime contexts of the preset (preset + pixel + wave + shape) as one 1M-cell array | Butterchurn `presetEquationRunner.js` `mdVSBase.gmegabuf = this.gmegabuf` | `src/core/expr.ts` `GMEGABUF_HOLDER` swap-and-restore + `MilkPresetRunner.gmegabuf` Float64Array(1048576) shared to every compiled Program | same | `tests/expr.test.ts` shared-gmegabuf tests (3 tests) | implemented | Cross-Program sharing, out-of-range clamping, `usesGmegabuf` compile-time detection asserted. Preserves `implemented` status only while the shared array persists across frames of the same preset. |
